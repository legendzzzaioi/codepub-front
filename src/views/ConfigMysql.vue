<template>
  <div class="container">
    <!-- 如果数据存在，则展示表格-->
    <a-table :data-source="mysqlconfiglist" v-if="mysqlconfiglist.length" row-key="id">
      <!-- 表格中的列 -->
    ...
    <!-- 更新数据的模态框 -->
    <a-modal v-model:open="isUpdateModalVisible" title="Update mysql Config" @ok="handleUpdate()"
             @onCancel="handleCancel">
      <a-form>
        <a-form-item label="Name">
          <a-input v-model:value="mysqlconfigreq.name"/>
        </a-form-item>
        <a-form-item label="Url">
          <a-input v-model:value="mysqlconfigreq.url" type="url"/>
        </a-form-item>
        <a-form-item label="Username">
          <a-input v-model:value="mysqlconfigreq.username" type="username"/>
        </a-form-item>
        <a-form-item label="Password">
          <a-input v-model:value="mysqlconfigreq.password" type="password"/>
        </a-form-item>
      </a-form>
    </a-modal>
    ...
    <a-space>
      ...
      <a-button @click="refreshMysqlConfigList">Refresh</a-button>
    </a-space>
  </div>
</template>
...
const showUpdateModal = (record: any) => {
  mysqlconfigreq.value = {name: record.name, url: record.url, username: record.username, password: ''};
  id.value = record.id;
...
// 展示新增模态框
const showAddModal = () => {
  mysqlconfigreq.value = {name: '', url: '', username: '', password: ''};
...
const handleAdd = async () => {
  if (mysqlconfigreq.value) {
    const result = await createMysqlConfigData(mysqlconfigreq.value);
    if (result.success) {
      await fetchMysqlConfigList();
      isAddModalVisible.value = false;
    }
  }
};
...
const handleUpdate = async () => {
  if (mysqlconfigreq.value) {
    ...
    const result = await updateMysqlConfigData(id.value, mysqlconfigreq.value);
    if (result.success) {
      await fetchMysqlConfigList();
      isUpdateModalVisible.value = false;
    }
...
const handleDelete = async (id: number) => {
  const result = await deleteMysqlConfigData(id);
  if (result.success) {
    await fetchMysqlConfigList(); // Ensure fetching updated data
  }
};
