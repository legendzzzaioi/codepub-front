<template>
  <div class="container">
    <!-- 如果数据存在，则展示表格-->
    <a-table :data-source="jenkinsconfiglist" v-if="jenkinsconfiglist.length" row-key="id">
      <!-- 表格中的列 -->
    ...
    <!-- 更新数据的模态框 -->
    <a-modal v-model:open="isUpdateModalVisible" title="Update jenkins Config" @ok="handleUpdate()"
             @onCancel="handleCancel">
      <a-form>
        <a-form-item label="Name">
          <a-input v-model:value="jenkinsconfigreq.name"/>
        </a-form-item>
        <a-form-item label="Url">
          <a-input v-model:value="jenkinsconfigreq.url" type="url"/>
        </a-form-item>
        <a-form-item label="Username">
          <a-input v-model:value="jenkinsconfigreq.username" type="username"/>
        </a-form-item>
        <a-form-item label="Password">
          <a-input v-model:value="jenkinsconfigreq.password" type="password"/>
        </a-form-item>
      </a-form>
    </a-modal>
    ...
    <a-space>
      ...
      <a-button @click="refreshJenkinsConfigList">Refresh</a-button>
    </a-space>
  </div>
</template>
...
const showUpdateModal = (record: any) => {
  jenkinsconfigreq.value = {name: record.name, url: record.url, username: record.username, password: ''};
  id.value = record.id;
...
// 展示新增模态框
const showAddModal = () => {
  jenkinsconfigreq.value = {name: '', url: '', username: '', password: ''};
...
const handleAdd = async () => {
  if (jenkinsconfigreq.value) {
    const result = await createJenkinsConfigData(jenkinsconfigreq.value);
    if (result.success) {
      await fetchJenkinsConfigList();
      isAddModalVisible.value = false;
    }
  }
};
...
const handleUpdate = async () => {
  if (jenkinsconfigreq.value) {
    ...
    const result = await updateJenkinsConfigData(id.value, jenkinsconfigreq.value);
    if (result.success) {
      await fetchJenkinsConfigList();
      isUpdateModalVisible.value = false;
    }
...
const handleDelete = async (id: number) => {
  const result = await deleteJenkinsConfigData(id);
  if (result.success) {
    await fetchJenkinsConfigList(); // Ensure fetching updated data
  }
};
